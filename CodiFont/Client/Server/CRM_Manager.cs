...
using TFG.Scripts.Server.Responses;
...
namespace TFG.Scripts.Server
{
    ...
    public class CRM_Manager : MonoBehaviour
    {
        ...
        private string[] _currentActiveItems = Array.Empty<string>();
        ...
        private void ApplyContract(string json)
        {
            ...
            // Obtenim els camps fixos de PlayerData mitjançant reflexió
            FieldInfo[] fixedFields = typeof(Data.PlayerData)
                .GetFields(BindingFlags.Public | BindingFlags.Instance);

            foreach (string inputName in contract.inputs)
            {
                // Comprovem si el camp ja existeix com a camp fix
                bool isFixed = false;
                ...
                // Si no existeix, el creem dinàmicament al diccionari extraFields
        ...
        private string BuildPlayerQuery()
        {
            var pd = Data.PlayerData.Instance;
            return "?playerLevel=" + pd.playerLevel
                                   + "&playerXP="    + pd.playerXP
                                   + "&playerClass=" + UnityWebRequest.EscapeURL(pd.playerClass ?? "")
                                   + "&daysPlayed="  + pd.daysPlayed;
        }
        ...
        private IEnumerator FetchShop()
        {
            ...
            using (UnityWebRequest req = UnityWebRequest.Get(_shopUrl + BuildPlayerQuery()))            
            {
                yield return req.SendWebRequest();
                if (req.result != UnityWebRequest.Result.Success)
                {
        ...
        private IEnumerator FetchDecorations()
        {
            ...
            if (cached != null) { ApplyDecorations(cached); yield break; }

            using (UnityWebRequest req = UnityWebRequest.Get(_decorationsUrl + BuildPlayerQuery()))
            {
                yield return req.SendWebRequest();
                if (req.result != UnityWebRequest.Result.Success)
                {
        ...
        private IEnumerator FetchPrice(string itemId)
        {
            ...
            // CRM_Manager.cs - preparació de la petició POST
            WWWForm form = new WWWForm();

            // Camps fixos via reflexió
            FieldInfo[] fixedFields = typeof(Data.PlayerData)
                .GetFields(BindingFlags.Public | BindingFlags.Instance);
